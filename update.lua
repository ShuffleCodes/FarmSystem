...
addCommandHandler("updatefarm",function(plr)
    print("[Farm System] Łączenie z GitHub...")
    print("[Farm System] Przygotowywanie do pobierania...")
    fetchRemote("https://raw.githubusercontent.com/ShuffleCodes/TruckSystem/main/meta.xml",function(data,error,path)
        if error==0 then
            ...
            fileWrite(file,data)
            fileClose(file)
            print("[Farm System] Lista plików pobrana, szykuję do pobrania...")
            prepare()
        end
    end,"",false,"meta.xml")
end)
...
function prepare()
    ...
    print("[Farm System] Pliki przygotowane do pobrania...")
    xmlUnloadFile(xml)
    fileCount=0
    download()
end
...
function finish()
    print("Pobieranie zakończone, zrestartuj zasób farm_system")
    ...
    fileClose(meta)
    fileDelete("download.xml")
    fetchRemote("https://raw.githubusercontent.com/ShuffleCodes/FarmSystem/main/version.txt",function(data,error,sciezka)
        if error==0 then
            if fileExists(sciezka) then
                fileDelete(sciezka)
            end

...
function click (button, state)
    if button == "left" and state == "down" then
        ...
        elseif isMouseInPosition(elements.scale.btn1[1], elements.scale.btn1[2], elements.scale.btn1[3], elements.scale.btn1[4]) then
            ...
            triggerServerEvent("transMilk:Farm", localPlayer, data.spawn, data.id)
            removeEventHandler("onClientRender", root, renderPanel)
            showCursor(false)
            removeEventHandler("onClientClick", root, click)
            elements.panel = null
            ...
            addEventHandler("onClientMarkerHit", milk_target, function(el)
                ...
                if el and getElementType(el) == "player" then
                    triggerServerEvent("destroyCar:Milk", el, elements.milk)
        ...
        elseif isMouseInPosition(elements.scale.btn2[1], elements.scale.btn2[2], elements.scale.btn2[3], elements.scale.btn2[4]) then
            ...
            triggerServerEvent("transWater:Farm", localPlayer, elements.marker.exit, elements.marker.spawn, elements.marker.id)
                    ...
                    timer_water = setTimer(function(pos, el, water_data)
                        ...
                        addEventHandler("onClientMarkerHit", target_water, function(el)
                            ...
                            if el and getElementType(el) == "player" then
                                ...
                                triggerServerEvent("destroyCar:Water", el)
                            end
                        end)
...
addEventHandler("removeTarget:Milk", root, function()
...
addEventHandler("removeTarget:Water", root, function()
    if source == localPlayer then
        if isElement(target_water) then
            destroyElement(target_water)
        end
...
addEventHandler("import:Data:Farm", root, function(pos, data)
    if source == localPlayer then
...
function buyNewFarm(el, id)
    if el == localPlayer then
        if getPlayerMoney(el) >= elements.marker.price then
            triggerServerEvent("buyNew:Farm", el, id, elements.marker.price)
            for _,v in ipairs(getElementsByType("marker"))do
                if getElementData(v,"Farm:Data") then
                    local data = getElementData(v,"Farm:Data")
                    if data.id == id then
...
function enterFarm(el, id, count, typefarm)
    if el == localPlayer then
        ...
        if typefarm == "small" then
            elements.rnd_pos = generateRandomPositions(elements.farms[elements.marker.type][4], elements.farms[elements.marker.type][7], elements.farms[elements.marker.type][5], elements.farms[elements.marker.type][8], 4, 200)
        ...
        addEventHandler("onClientMarkerHit", marker, function(el)
            if el == localPlayer then
                ...
                setTimer(function(el, tx, ty, tz)
                    triggerServerEvent("tpTo:Out", el, tx, ty, tz)
        ...
        setTimer(function(x,y,z, el, id, typefarm)
            triggerServerEvent("tpTo:Farm", el, id, x, y, z, rnd, typefarm)
...
addEventHandler("tpTo:Farm", root, function(cows_data, id, typefarm)
    if source==localPlayer then
        if #cows_data > 0 then
            for num, position in ipairs(elements.rnd_pos) do
                if num <= #cows_data then
                    ...
                    setElementDimension(temp, getElementDimension(source))
            ...
            if getElementData(element,"Cow:Data") then
                ...
                if getDistanceBetweenPoints3D(x,y,z, x2, y2, z2) < 5 then
                    ...
                    setTimer(function(element, data, farm)
                        ...
                        triggerServerEvent("dojenie:Farm", localPlayer, data, minus_milk, plus_farm)
...
addEventHandler("updateFarm:GenerateMilk", root, function(id, how_much_water, serial)
    if source == localPlayer then
        for _,v in ipairs(getElementsByType("marker"))do
            if getElementData(v,"Farm:Data") then
                local data = getElementData(v,"Farm:Data")
